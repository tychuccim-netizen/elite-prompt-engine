...
# 1. 網頁頂層旗艦商務配置
st.set_page_config(
    page_title="日月鑫管顧 | 商業視覺資產工廠 5.2",
    page_icon="🏭",
    layout="wide"
)

# 企業品牌大器標題
st.title("🏭 日月鑫商務視覺資產工廠 5.2")
st.subheader("解構《數位時代》5大商業維度 × 中英雙語 AI 專用結構化標籤鏈結系統")
st.markdown("本系統已全面修復 Action 欄位的語法錯誤，並維持無國旗標識之純淨商務排版。")
st.markdown("---")

# 2. 11×10 跨界商業資產大數據矩陣數據庫 (語法完整校正版)
matrix = {
    "Dimension": {
    ...
    "Where": {
        ...
        "options": {
            ...
            "10": {"ch": "文青風手繪溫慢奶油色留白背景底層", "en": "against a cozy warm cream-colored hand-drawn blank canvas"}
        }
    },
    "Light": {
        "name": "09. 光線氛圍 (Light)",
        "options": {
            ...
            "2": {"ch": "溫慢黃金黃昏逆光、柔和耀光", "en": "golden hour warm backlighting with soft sun flares"},
    ...
    "Camera": {
        "name": "10. 鏡頭/構圖視角 (Camera)",
        "options": {
            "1": {"ch": "聚焦特寫鏡頭", "en": "close-up shot, sharp focus"},
            "2": {"ch": "標準商務中景鏡頭", "en": "medium shot, professional corporate framing"},
            "3": {"ch": "大器環境廣角鏡頭", "en": "wide-angle shot showing majestic environment"},
            "4": {"ch": "85mm 大光圈經典人像鏡", "en": "shot on 85mm lens, f/1.4, shallow depth of field, creamy blurred background"},
            "5": {"ch": "35mm 人文寫實紀錄鏡頭", "en": "35mm lens documentary style, authentic storytelling"},
            "6": {"ch": "低角度英雄氣勢仰拍", "en": "low-angle heroic shot, powerful perspective"},
            "7": {"ch": "大師級高角度俯拍位置", "en": "high-angle dramatic cinematic placement"},
            "8": {"ch": "長焦空間壓縮鏡頭技術", "en": "telephoto lens compression effect"},
            "9": {"ch": "2.35:1 電影變形寬螢幕", "en": "anamorphic widescreen aspect ratio shot"},
            "10": {"ch": "黃金比例完美平衡構圖", "en": "perfect golden ratio composition layout"}
        }
    },
    "Format": {
        "name": "11. 輸出格式與後綴參數 (Format)",
        "options": {
...
order = ["Dimension", "Style", "Race", "Who", "Look", "Wear", "Action", "Where", "Light", "Camera", "Format"]
selections = {}

col_group1 = st.columns(4)
col_group2 = st.columns(4)
col_group3 = st.columns(3)
all_cols = col_group1 + col_group2 + col_group3
...
if st.button("🎲 啟動 11×10 百億級別全領域跨界創新配置"):
...
ch_formula = (
    ...
    f"動態特徵為【{selections['Action']['ch']}】，環境設定在【{selections['Where']['ch']}】。畫面融入【{selections['Light']['ch']}】，"
    f"並搭配【{selections['Camera']['ch']}】，最終以【{selections['Format']['ch']}】高規格交付。"
)
...
with col_output_ch:
    ...
    ch_prompt = (
        ...
        f"{selections['Where']['ch']}, {selections['Light']['ch']}, {selections['Camera']['ch']}, {selections['Format']['ch']}, "
